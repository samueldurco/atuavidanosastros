...
## Continuidade no dashboard

O dashboard autenticado recupera a RPC natal e a Biblioteca independentemente, com timeout de dez segundos por leitura. Serializa somente estado natal e precisão da hora, nunca data, local, coordenadas, revisão ou recibos. Falha/retorno inválido é `UNAVAILABLE`, não cadastro ausente; erro na Biblioteca não exibe simultaneamente uma Biblioteca vazia. O resumo direciona para iniciar, retomar, recuperar ou revisar `/conta/nascimento` e mantém a incerteza de hora aproximada visível.

Não cria leitura, assinatura, entitlement, porcentagem de progresso nem execução automática. Preview sem autenticação configurada é explícito e não consulta dados. Respostas são privadas, sem cache/indexação/referrer. QA local: `docs/qa/DASHBOARD_NATAL_CONTINUITY_2026-09-24.md`.
