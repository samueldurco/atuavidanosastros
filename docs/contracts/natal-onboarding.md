...
Status: contrato/API/persistência local. Não é onboarding visual entregue, certificação do motor ou autorização de processamento. Nenhum dado real deve ser usado na validação.
...
`supabase/forward-fixes/disable_natal_onboarding_writes.sql` suspende escritas mantendo recuperação e dados. Não restaura bypass. Testado contra banco local real. Implantação hospedada, JWT/PostgREST real, geocodificação, interface ID-02/Stitch, captura de consentimento visual, retomada do formulário, integração dashboard/produtos e exclusão integral da conta são pendentes. Todos os gates de produto/editorial permanecem inalterados.
