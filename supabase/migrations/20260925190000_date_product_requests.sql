...
-- Private lineage/consent receipt; target date is account-associated, not telemetry.
create table public.date_product_requests (
...
alter table public.date_product_requests enable row level security;
revoke all on public.date_product_requests from public, anon, authenticated, service_role;

create function public.request_date_product_run(p_request_key uuid, p_command jsonb)
...
begin
  ...
  if (select count(*) from jsonb_object_keys(p_command)) <> 5
    or exists(select 1 from jsonb_object_keys(p_command) k where k not in ('version','productId','expectedRevision','targetDate','consent'))
    or p_command->>'version' is distinct from 'atv-date-request/1'
    or p_command->>'productId' is distinct from 'date-reading'
  ...
  if v_target < date '1900-01-01' or v_target > date '2099-12-31'
    or to_char(v_target, 'YYYY-MM-DD') is distinct from p_command->>'targetDate'
    then raise exception 'invalid_input'; end if;

  -- Same owner lock/order as generic and natal requests, then onboarding profile lock.
  ...
  select r.id, n.command into v_run, v_receipt from public.product_runs r
    left join public.date_product_requests n on n.run_id = r.id
  ...
  v_input := jsonb_build_object('version','atv-workflow/1.0.0', 'productId','date-reading',
  ...
  -- Existing release, entitlement, quota and atomic immutable input/event/Library transaction.
  v_run := public.request_product_run('date-reading',p_request_key,v_input,null);
  insert into public.date_product_requests(run_id,command,natal_version)
    values(v_run,p_command,(v_natal->>'version')::integer);
  return v_run;
end;
$$;
revoke all on function public.request_date_product_run(uuid,jsonb) from public, anon, service_role;
grant execute on function public.request_date_product_run(uuid,jsonb) to authenticated;
commit;
