...
import { parseNatalRequestInput } from '$lib/natal-request';
...
export async function natalRequestApi(
	...
	try {
		...
		const input = parseNatalRequestInput(body.input);
		if (!input) return fail('invalid_input', 400);
		const { data, error } = await client.rpc('request_natal_product_run', {
