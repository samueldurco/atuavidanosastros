...
import { parsePairRequestInput } from '$lib/pair-request';
...
export async function pairRequestApi(
	...
	try {
		...
		const input = parsePairRequestInput(body.input);
		if (!input) return fail('invalid_input', 400);
		const { data, error } = await client.rpc('request_pair_product_run', {
